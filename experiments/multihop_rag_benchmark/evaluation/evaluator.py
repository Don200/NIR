...
class EvaluationResult:
    ...
    def to_dict(self, include_context: bool = False) -> Dict[str, Any]:
        ...
        for p in self.predictions:
            ...
            if include_context:
                pred_dict["context"] = p.context
                pred_dict["retrieval_metadata"] = p.retrieval_metadata
    ...
    def save(self, path: Path, include_context: bool = False) -> None:
        """Save results to JSON file."""
        path.parent.mkdir(parents=True, exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            json.dump(self.to_dict(include_context=include_context), f, indent=2, ensure_ascii=False)
    ...
    def save_predictions_csv(self, path: Path) -> None:
        ...
        with open(path, "w", encoding="utf-8", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([
                "query_id", "query_type", "is_correct",
    ...
    def save_all(self, output_dir: Path, include_context: bool = True) -> None:
        """Save all results: JSON with full data + CSV for quick review."""
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)

        # Full JSON with context
        self.save(output_dir / f"{self.method_name}_results.json", include_context=include_context)

        # CSV for easy viewing
        self.save_predictions_csv(output_dir / f"{self.method_name}_predictions.csv")
